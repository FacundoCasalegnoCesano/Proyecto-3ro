...
async function checkStock(cartItems: CartItem[]): Promise<StockCheckResult> {
  ...
  for (const item of cartItems) {
    try {
      ...
      console.log(
        `📊 Producto ${item.id}: stock=${product?.stock}, solicitado=${item.quantity}`
      );
      ...
      console.error(
        `❌ Error verificando stock para producto ${item.id}:`,
        error
      );
      outOfStockItems.push({
        id: item.id,
...
async function updateProductStock(
  ...
  try {
    for (const item of cartItems) {
      console.log(
        `🔄 Actualizando stock: Producto ${item.id} - reduciendo ${item.quantity} unidades`
      );
      ...
      console.log(
        `✅ Stock actualizado: ${item.name} - nuevo stock: ${updatedProduct.stock}`
      );
    }
    return { success: true };
  } catch (error) {
