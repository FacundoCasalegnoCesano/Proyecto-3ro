...
interface RawProduct {
  ...
  image?: string;
  src?: string;
  tamaño?: string | boolean;
  cantidad?: string | boolean;
...
export function ProductsGridPage({
  ...
  const capitalizarPalabras = useCallback(
    (texto: string | boolean | undefined | null): string => {
  ...
  const tieneLineaEspecifica = useCallback(
    (linea: string | boolean | undefined | null): boolean => {
  ...
  const tieneMarcaEspecifica = useCallback(
    (marca: string | boolean | undefined | null): boolean => {
  ...
  const tieneAromaEspecifico = useCallback(
    (aroma: string | boolean | undefined | null): boolean => {
  ...
  const tieneCategoriaEspecifica = useCallback(
    (categoria: string | boolean | undefined | null): boolean => {
  ...
  const puedeSerAgrupado = useCallback(
    ...
    [
      tieneMarcaEspecifica,
      tieneCategoriaEspecifica,
      tieneLineaEspecifica,
      tieneAromaEspecifico,
    ]
  );
  ...
  const categoriaNoDebeAgruparse = useCallback(
    (categoria: string | undefined): boolean => {
      if (!categoria || typeof categoria !== "string") return false;
      const cat = categoria.toLowerCase().trim();

      // Lista de categorías que NO deben agruparse
      ...
      return categoriasNoAgrupables.some((catNoAgrupable) =>
        cat.includes(catNoAgrupable)
      );
    },
    []
  );
      ...
      Object.entries(grouped).forEach(([key, groupedProducts]) => {
        if (groupedProducts.length > 0) {
          ...
          const imagenRepresentativa =
            representative.image || representative.src;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {paginatedProducts.map((product) => (
          <ProductCard
            ...
            product={product}
            marcaSeleccionada={
              typeof product.marca === "string" ? product.marca : undefined
            }
            lineaSeleccionada={
