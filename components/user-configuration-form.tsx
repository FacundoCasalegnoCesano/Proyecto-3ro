...
export function UserConfigurationForm() {
  ...
  const [initialAddressData, setInitialAddressData] =
  ...
  const validatePostalCode = useCallback(
    (code: string, country: string): boolean => {
      ...
      const loadUserAddress = async () => {
        ...
        try {
          ...
          if (response.status === 401) {
            console.log(
              "No autorizado - la sesión puede no estar disponible en el servidor"
            );
          ...
          if (!response.ok) {
            ...
            if (response.status === 404) {
              console.log(
                "Usuario no encontrado en BD, usando valores por defecto"
              );
              const defaultAddress = {
                calle: "",
                ciudad: "",
  ...
  const validateForm = (): boolean => {
    ...
    if (hasAnyAddressData) {
      if (!calle.trim()) {
        newErrors.calle =
          "La dirección es obligatoria si completas otros datos";
      }
  ...
  const hasChanges = (): boolean => {
    ...
    return (
      JSON.stringify(formData.address) !== JSON.stringify(initialAddressData)
    );
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-8 p-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mb-4">
            Esta información se obtiene de tu perfil y no puede ser modificada
            aquí.
          </p>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    formData.address.pais === "Argentina"
                      ? "1234"
                      : formData.address.pais === "Brasil"
                      ? "12345-678"
                      : formData.address.pais === "Chile"
                      ? "1234567"
                      : "12345"
                  }
                />
