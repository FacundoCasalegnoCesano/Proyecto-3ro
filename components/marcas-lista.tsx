...
const transformBrandContactToRequest = (
...
export function MarcasLista({
  ...
  const [selectedRequest, setSelectedRequest] = useState<BrandRequest | null>(
    null
  );
  ...
  const handleUpdateStatus = useCallback(
    async (requestId: string, newStatus: UIBrandStatus) => {
      try {
  ...
  const stats = useMemo(() => {
    ...
    const contacted = brandRequests.filter(
      (r) => r.status === "contacted"
    ).length;
    const approved = brandRequests.filter(
      (r) => r.status === "approved"
    ).length;
    const rejected = brandRequests.filter(
      (r) => r.status === "rejected"
    ).length;
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Gestiona las marcas que quieren trabajar contigo
            </p>
          </div>
        </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleUpdateStatus(request.id, value as UIBrandStatus)
                    }
                  >
                    <SelectTrigger
                      className="flex-1"
